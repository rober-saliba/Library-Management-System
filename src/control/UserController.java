...
public class UserController implements IClient {
	 ...
	 * messages to the server. userSingleton - the single instance of this class sem
	 * - a semaphore that blocks the main thread until a response is received from
	 * the server. semaphore is acquired each time a request to the server is sent
	 * and released after the response arrives. currentUser, logInResult,
	 * logoutResult, updateSettinResult, Msg - different variables of different
	 * types that store the return results from the server.
	 */
	 ...
	 * @return a MsgParser object containing a User object if it exists and a login
	 *         result (Wrong password, already logged in, user doesn't exist).
	 ...
	 * @throws InterruptedException - thrown should acquiring the semaphore
	 *                              encounter a problem
	 */
