...
public class MsgController implements IClient {
	 ...
	 * messages to the server. messageSingleton - the single instance of this class
	 * sem - a semaphore that blocks the main thread until a response is received
	 * from the server. semaphore is acquired each time a request to the server is
	 * sent and released after the response arrives. msgResult, NumOfMessages,
	 * deleteResult - different variables of different types that store the return
	 ...
	 * @return an ArrayList of Messages containing all messages returned from the
	 *         server.
	 */
	...
	/**
	 * sends a request to the server to get number of messages of a specific
	 * librarian.
	 * 
	 * @param belong - the ID of the librarian
